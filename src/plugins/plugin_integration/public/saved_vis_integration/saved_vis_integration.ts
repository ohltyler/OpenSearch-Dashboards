...
import { createSavedVisIntegrationClass } from './_saved_vis_integration';
...
export interface SavedObjectOpenSearchDashboardsServicesWithVisIntegration
  extends SavedObjectOpenSearchDashboardsServices {}
export type SavedVisIntegrationLoader = ReturnType<typeof createSavedVisIntegrationLoader>;
export function createSavedVisIntegrationLoader(
  services: SavedObjectOpenSearchDashboardsServicesWithVisIntegration
) {
  const { savedObjectsClient } = services;

  class SavedObjectLoaderVisIntegration extends SavedObjectLoader {
  ...
  const SavedVisIntegration = createSavedVisIntegrationClass(services);
  return new SavedObjectLoaderVisIntegration(
    SavedVisIntegration,
    savedObjectsClient
